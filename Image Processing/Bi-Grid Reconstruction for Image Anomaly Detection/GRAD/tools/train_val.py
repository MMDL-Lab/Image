...
def main():
    ...
    auto_resume = config.saver.get("auto_resume", True) # False
    resume_model = config.saver.get("resume_model", None) # None
    ...
    if resume_model:
        ...
        load_state(load_path, model, optimizer=optimizer)   
...
def train_one_epoch(
    ...
    for i, input in enumerate(train_loader):
        if cur_epoch >= 20:
            ...
            input['mode'] = 'train_normal'
            # forward
            outputs = model(input)
            loss = 0

            for name, criterion_loss in criterion.items():
                weight1 = criterion_loss.weight
                loss += weight1 * criterion_loss(outputs)
            ...
            if (curr_step + 1) % config.trainer.print_freq_step == 0 and rank == 0:
                ...
                logger.info(
                    ...
                    "Rec Loss {rec_loss.val:.5f} ({rec_loss.avg:.5f})\t".format(
                        epoch + 1,
            ...
            # measure data loading time
            data_time.update(time.time() - end)
            input['mode'] = 'train_abnormal'
            # forward
            outputs = model(input)
            loss = 0

            for name, criterion_loss in criterion.items():
                loss += 0.5 * outputs['con_loss'] + 0.5 * criterion_loss(outputs)
...
def validate(val_loader, model):
    ...
    if rank == 0:
        ...
        # evaluate, log & vis
        ret_metrics, image_preds = performances(fileinfos, preds, masks, config)
        threshold = log_metrics(ret_metrics, config.evaluator.metrics)
